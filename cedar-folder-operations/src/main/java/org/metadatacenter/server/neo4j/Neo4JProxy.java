...
public class Neo4JProxy {
  ...
  FolderServerFolder createFolderAsChildOfId(String parentId, String name, String displayName, String description, String
  ...
  private FolderServerFolder buildFolder(JsonNode f) {
  ...
  private FolderServerNode buildNode(JsonNode f) {
  ...
  private FolderServerResource buildResource(JsonNode r) {
  ...
  private FolderServerUser buildUser(JsonNode u) {
  ...
  private FolderServerGroup buildGroup(JsonNode g) {
  ...
  FolderServerGroup createGroup(String groupURL, String name, String displayName, String description, String ownerURL,
  ...
  public FolderServerGroup findGroupBySpecialValue(String specialGroupName) {
  ...
  boolean removeOwner(FolderServerFolder folder) {
  ...
  boolean setOwner(FolderServerResource resource, FolderServerUser user) {
  ...
  boolean setOwner(FolderServerFolder folder, FolderServerUser user) {
  ...
  public void addPermissionToGroup(String nodeURL, String groupURL, NodePermission permission, boolean nodeIsFolder) {
    ...
    if (group != null) {
      if (nodeIsFolder) {
        FolderServerFolder folder = findFolderById(nodeURL);
        if (folder != null) {
          addPermission(folder, group, permission);
        }
      } else {
        FolderServerResource resource = findResourceById(nodeURL);
        if (resource != null) {
  ...
  public void addPermissionToUser(String nodeURL, String userURL, NodePermission permission, boolean nodeIsFolder) {
    FolderServerUser user = findUserById(userURL);
    if (user != null) {
      if (nodeIsFolder) {
        FolderServerFolder folder = findFolderById(nodeURL);
        if (folder != null) {
